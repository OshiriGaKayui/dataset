...
def learn(dftrain,dftest):
    ...
    rare_words_list=get_rare_words(df1)
    ...
    (tfidf, X)=create_features(df1,rare_words_list)
    ...
    def create_labels(processed_tweets):
        ...
        df.loc[df['screen_name'].isin(['realDonaldTrump', 'mike_pence', 'GOP']), 'label'] = 'Right'

        #return np.array(df['label'],dtype=object)
        return df


    df1=create_labels(df1)
    ...
    SModel=learn_classifier(X,df1['label'])
    ...
    return classify_tweets(tfidf,SModel,dftest)

dftrain=pd.read_csv("tweets_train.csv")
dftest=pd.read_csv("tweets_test2.csv")

print(learn(dftrain,dftest))
